...
Approved 2026-09-13 in desktop task. Feature branch: `feature/dice-styles-and-throw-continuity`. Integrated Claude’s main update `25a0c0b` in branch merge `677f683`; main is not modified. User then prioritized analysing the OBS recording `2026-09-13 14-54-37.mp4` and fixing the persistent apparent throw reset before resuming features. Keep this file current for Claude handoff. The earlier PLAY-ROADMAP is separate; its unfinished phases are not automatically complete.
...
The recording was inspected at 2 fps, then 10 fps around 16 seconds. At 16.0–16.1 seconds the die jumps as the new rolling display appears. The server acknowledgement replaced the local live throw with a replay from release. The fix retains the live world and elapsed steps for matching releases and same-ID refreshes; settled notification follows final correction.
