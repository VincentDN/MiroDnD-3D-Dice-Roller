...
- Main push triggers normal CI build/release/Cloudflare deployment; check that run before claiming production delivery. Local runtime required removing inherited ELECTRON_RUN_AS_NODE and making npm available on PATH for packaging; source needs no workaround for those machine settings.
