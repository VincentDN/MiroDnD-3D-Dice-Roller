# Temporary roll effects: advantage/disadvantage, Bless/Guidance/Bardic Inspiration (2026-09-17, third follow-up session)

- User: "resume the work." Confirmed the hidden-rolls push (below) deployed cleanly (all 4 CI jobs green) before picking the next item: `PLAY-ROADMAP.md`'s #3 / `GRAND-ROADMAP.md` Horizon 1.1, the last substantial item left in Horizon 1 besides connection resilience and session history.
- Entirely client-side, zero server/schema changes - `lib/effects.ts` is a pure transform: given an expression and the current `EffectsState` (an advantage/disadvantage mode plus a list of named bonus-die effects), it returns the final expression to actually send, `changed` (for the preview), and which one-use effect ids it spent. The server never learns "Bless" exists, same as how the free-form roll box already lets a player type any dice notation - this is just another way to arrive at one.
- **Composes with saved actions, not just the free-form box, without touching `ActionBar`**: `app/page.tsx`'s `roll()` callback applies effects centrally to every roll path (free-form, QuickRoll, and every `ActionBar`/`Soundboard` `onRoll` call, since they all funnel through the same function) - gated by `!options?.linkedTo`, so a linked damage roll is categorically excluded, never guessed at from the expression shape. Verified end-to-end: an attack action's primary roll picks up Bless's `+1d4`, its damage roll from the same click sequence does not.
- Advantage/disadvantage deliberately only transforms a *plain* `1d20[+-N]` expression (regex-matched) - a saved action already shaped like `2d20kh1+5` (Barbarian's Reckless Attack, say) is left alone rather than double-advantaged, and that's visible in the same live preview line (`Will roll <code>...</code>`) rather than a silent no-op.
- One-use consumption is gated on the roll's request having genuinely resolved (inside the `try` block's success path, after `api()` returns) - a failed attempt never spends anything, and a retry of the same still-in-flight/failed request can't double-spend either, since consumption only ever fires once, on eventual success.
- `components/effects-panel.tsx` exports `useEffects(room, playerId)` (called once in `app/page.tsx`, mirroring the existing `useSettings()` hook-once/pass-down pattern) plus the panel itself - room+player-scoped localStorage (`rollparty:effects:<room>:<player>`), the same scoping as the roll notebook, since these are temporary in-game conditions for one character in one session, not a device-wide preference. Three built-in presets (Bless persistent, Guidance and Bardic Inspiration one-use) plus a small add-your-own form (name, die like `1d4`/`2d6`, persistent toggle) for homebrew effects, capped at 8.
- Validation: 54 web unit tests (new: `tests/effects.test.ts`, 9 tests - transform correctness, persistent-vs-one-use consumption, validation/round-trip), TypeScript, production build, and the full browser suite (14/14, including new `tests/browser/effects.spec.ts`) all pass in one clean run, both previously-noted-flaky physics tests included. Manually verified via a throwaway script mirroring the eventual test: preview text, the actual network request body, Bless surviving a roll, Guidance deactivating after one, and the attack-vs-damage split - all matched expectations exactly (one early false alarm was the verification script itself double-toggling Bless, not an app bug - noted here so a future agent doesn't waste time rediscovering that).
- Not done, deliberately: no server-side validation of effect dice (they're free-form client state, validated only by the existing `parseExpression` the final combined string already goes through) - a malformed custom effect just fails to parse like any other bad expression would, same UX as today. No UI to see "what effects the DM/other players have active" - purely personal, per the roadmap's framing of these as one character's temporary conditions.

---

# Hidden ("DM only") rolls (2026-09-17, second follow-up session)

- User: "resume working down the roadmap." Picked Horizon 1.3 from `LLM-Docs/GRAND-ROADMAP.md` (also `PLAY-ROADMAP.md`'s #5) - hidden rolls, flagged in both as the single most-requested missing feature and a prerequisite for later DM tooling. Confirmed via GitHub Actions that the prior two pushes (party roles, music player) both fully deployed to production (all four jobs green, including "Update live Cloudflare site") before starting this.
- Any player can check "Hide from party" (or the eye icon in the desktop hotbar) on a roll; a new optional `visibility?: 'dm'` field on `Roll` (`lib/dice.ts`) and `ActionRollOptions` (`lib/action-damage.ts`) carries the request through the existing `roll()`/`ActionBar onRoll` plumbing unchanged - no new endpoint needed for making one, only for reading and revealing them.
- **Real server-side filtering, not UI hiding** - the actual security property this feature exists for: `app/api/session/route.ts`'s `GET` now resolves the requesting player from `x-player-key` (`viewerFor`) and filters every hidden roll out of the response via `lib/roll-visibility.ts`'s pure, unit-tested `visibleTo()`, unless the viewer is the DM or the roll's own owner. An unauthenticated request (no valid player key - an OBS/overlay spectator link) resolves no viewer and so never sees a hidden roll either. This is the same shape as the DM-only enforcement already established for music (`player.role==='dm'`), reused rather than building the room-owner/permission primitive `PLAY-ROADMAP.md` originally called for - deliberately, see below.
- Closed two real leak vectors the acceptance criteria implied but the obvious implementation would have missed: (1) re-throwing/dragging an existing roll (`action:'throw'` with a `parent`) was never scoped to the requesting player - added a check that a hidden parent can only be picked up by its owner or the DM, with the same generic "no longer available" error either way so a disallowed player can't distinguish a hidden roll from a nonexistent one; (2) linked damage rolls now inherit their attack's `visibility` (a damage roll for a secret attack stays just as secret) rather than defaulting to public.
- `action: 'reveal'` is a new, explicit, one-way action (owner or DM only, 400 if already public) - never inferred, matching the roadmap's "revealing creates a deliberate public event" requirement. `components/roll-history.tsx` shows a "Hidden" badge plus a Reveal button (only to those who can use it); the tray's "Last Roll" label gets the same badge.
- Validation: 45 web unit tests (new: `tests/roll-visibility.test.ts`, 5 tests, covering every viewer/owner/DM/unresolved combination), TypeScript, production build, and `tests/session.mjs` grew a full leak-prevention suite against real local D1 - hidden-from-a-second-player, hidden-from-an-unauthenticated-GET, wrong-player-can't-reveal, wrong-player-can't-rethrow, linked-damage-inherits-hidden, reveal-then-visible-to-everyone, and reveal-twice-rejected. New `tests/browser/hidden-rolls.spec.ts` (three real browser contexts: roller, DM, third player) plus the full existing suite all pass - 13/13 in one clean run including both of this sandbox's previously-flaky physics tests, which happened to behave this run (see the 2026-09-17 music-player entry below for why an occasional failure there isn't a regression).
- Not done, deliberately, to stay scoped: no per-saved-action hide toggle (ActionBar/Soundboard buttons aren't wired to `visibility` - only the free-form Roll/QuickRoll form and desktop hotbar are); no dedicated room-owner/permission primitive (reused the existing `role==='dm'` check, same as music - a real gap if a room ever needs more than one authority level, worth building generally whenever a third feature wants it).

---

# Shared DM-controlled music player (2026-09-17, follow-up session)

- User request, first item off the new `LLM-Docs/GRAND-ROADMAP.md`: "a simple music player - the DM can point it at a folder full of mp3s, bookmark tracks or shuffle, players can only mute or set volume." Confirmed with the user that this means genuinely synced playback (not DM-machine-only ambiance): the DM's track/shuffle/bookmark choices broadcast to the room; any client (DM or player) who separately points their own browser at a matching local folder hears it in sync.
- No server-side audio hosting, per the roadmap's own guardrails - `lib/music.ts` defines a small shared state (`trackId`/`trackName`/`playing`/`position`/`shuffle`/`bookmarks`, server-stamped `updated`) stored as JSON in a new nullable `rooms.music` column (`drizzle/0003_dizzy_songbird.sql`). The server never sees or stores actual audio, only an opaque filename-derived track id.
- `lib/music-library.ts` + new ambient types in `lib/file-system-access.d.ts` (the File System Access API isn't in this repo's TypeScript DOM lib yet) wrap `showDirectoryPicker`/directory listing/IndexedDB handle persistence - purely client-side, feature-detected (`supported()`), gracefully hidden on non-Chromium browsers.
- `components/music-player.tsx` resolves the shared `trackId` against whatever local folder *this* device has chosen, computing live position from the server-stamped state (`lib/music.ts`'s `livePosition()`) rather than trusting any client clock. DM gets folder/transport/shuffle/bookmark controls; everyone gets volume/mute (local-only, never a network call - verified explicitly in `tests/browser/music.spec.ts`).
- **Real server-side enforcement, not just UI hiding**: `action: 'music'` in `app/api/session/route.ts` checks the authenticated player's stored `role === 'dm'` (the column added in the party-roles work below) and 403s otherwise - reusing that existing column meant this didn't need the room-owner/permission system `PLAY-ROADMAP.md`/`GRAND-ROADMAP.md` still list as future work.
- **Found and ruled out, not fixed - a real pre-existing bug, unrelated to this feature**: `tests/browser/roller.spec.ts`'s "briefing intro" test (and likely `starting-die.spec.ts`'s browser-mode variant) fails reproducibly - not just under sandbox load - because after the lobby's practice-dice drag-and-drop, the *room's* `DiceStage` canvas stops responding to a second `setViewportSize` change (verified with a throwaway diagnostic script: the canvas keeps its 1440px-wide dimensions after resizing to 390px, when a room-only flow with no prior practice drag resizes correctly both times). Confirmed via `git stash` that this reproduces identically on the commit from *before* this session's music-player work even started - it is not a regression from anything below. Left unfixed deliberately: it lives in `components/dice-stage.tsx`'s ResizeObserver/ WebGL lifecycle, which has a long, carefully-tuned history (see the many physics/rendering entries elsewhere in this file) and touching it wasn't in scope for a "simple music player." Next agent: reproduce via the lobby-drag-then-room-resize sequence before assuming any given browser-suite failure is ordinary resource-contention flakiness.
- Validation: 40 web unit tests (new: `tests/music.test.ts`, 7 tests), TypeScript, production build, local D1 migration (`0003...`) applied cleanly, `tests/session.mjs` (new DM-only/validated/server-timestamped music assertions), and the full Playwright browser suite pass except the pre-existing bug above (independently reproduced on pre-session `main`, not caused by this work) - `tests/browser/music.spec.ts` itself (mocking `window.showDirectoryPicker` with an in-memory fake folder, since a real OS picker can't be automated headlessly) passes reliably in isolation and inside the full suite.
- Not done: no recursive subfolder scanning (flat folder listing only), no seek-bar scrubbing beyond play/pause/prev/next, no crossfade. The `GRAND-ROADMAP.md` guardrails (no external audio hosting, no mandatory account, server never trusts the client) were followed throughout.

---

# Party roles, class hotbars, DM soundboard, quickroll rattle (2026-09-17)

- User request: remove player tokens/portraits and all "Dungeons of Drakkenheim" identifiers, replace with generic party roles (DM/Barbarian/Wizard/Cleric/Monk/Create Player), seed each class with a color/gradient-mapped starter hotbar, add a quick edit affordance + drag-to-reorder to saved actions, give the DM a d20±1..5 row plus a fun soundboard, and give Quickroll its own rolling-dice sound before the existing result cue. Full plan in `LLM-Docs/PARTY-ROLES-ROADMAP.md`. User explicitly authorized pushing to main.
- `lib/roles.ts` replaces `lib/avatars.ts`; `components/role-picker.tsx` replaces `components/avatar.tsx` (icon badges via lucide, no portrait files - `public/avatars/*.png` deleted). `custom` (Create Player) picks a raw hex directly. `db/schema.ts`'s `players.avatar` column is renamed to `role` via `drizzle/0002_married_lord_hawking.sql` (`ALTER TABLE ... RENAME COLUMN`); old avatar ids in existing rows just fall back to the pre-existing unrecognized-role letter badge, no data loss. Server derivation lives in `app/api/session/route.ts`'s `roleAndColor()`.
- `lib/role-presets.ts` + `lib/action-profiles.ts`'s new `ensureRoleProfile()` seed a role's default hotbar (attack+linked damage, saves, a check, class-gradient colors) into local storage exactly once per role, the first time it's picked, and switch to it - re-picking never re-seeds or clobbers edits (tagged via a new `roleId` field on `ActionProfile`, independent of the user-editable name). The DM's set is literally `d20+1..+5`/`d20-1..-5`, shaded green→red by magnitude.
- `components/action-bar.tsx`: every hotbar button now also carries a small pencil "Quick edit" button (distinct aria-label from the existing manage-list "Edit" button - a real bug caught by the browser suite: both starting with an identical accessible name broke `action-bar.spec.ts`/`appearance.spec.ts`'s exact-name lookups) that opens Manage actions and scrolls straight to the editor. The manage-list rows are now HTML5 drag-and-drop reorderable (grip handle) alongside the existing up/down buttons.
- `lib/dice-audio.ts`: `confirmedSound()` now plays a short synthesized "dice rattle" before the ping/fanfare when `roll.quick` is set (physical rolls unchanged - their animation already carries impact sounds). New `SOUND_EFFECTS`/`playSoundEffect()` (drumroll, dramatic sting, applause, sad trombone, rimshot) back a new `components/soundboard.tsx`, shown only for the DM role - same synthesized-only approach as every other sound in this file, no audio files.
- Renamed the `drakkenheim` theme id to `toxic` everywhere (`lib/settings.ts`, `app/globals.css`, `app/layout.tsx`'s flash-prevention script) - same acid-green palette, just a non-campaign-branded identifier. Removed the lobby's "DnD Sundays 2026: Dungeons of Drakkenheim" heading and "STAR GODS HUNGER" dice-strip line for generic copy.
- **Real bug found and fixed via the browser suite, not by inspection**: the new default "Create Player" color was initially a teal (`#32a6c8`), but `tests/browser/throw-continuity.spec.ts` tracks the thrown die by reading back canvas pixels matching a specific purple/magenta hue (the old default avatar's `#c44dff`) - teal doesn't match that heuristic, so `n` (matched pixel count) was 0 and the test's centroid math produced `NaN`. Fixed by making `DEFAULT_CUSTOM_COLOR` the app's own purple accent (`#c44dff`, already `--primary` in `app/globals.css`) instead of an arbitrary color - also more on-brand. Confirms this specific test's pixel-tracking approach is sensitive to the exact default hue; keep that in mind if the default ever changes again.
- Validation: 33 web unit tests (new: `tests/roles.test.ts`, extended `tests/action-profiles.test.ts` for `ensureRoleProfile`), 8 desktop tests, TypeScript, production build, local D1 migration (`drizzle/0002...`) applied cleanly, `tests/session.mjs` integration (updated for `role`/`color` instead of `avatar`), and the full Playwright browser suite (11/11) all pass - verified both via `node scripts/test-integration.mjs --browser` and by manually re-running individually-flaky tests in isolation to rule out sandbox resource-contention flakiness (documented elsewhere in this file as a recurring, pre-existing pattern in this sandbox) rather than a real regression. Manually screenshotted the role picker, Barbarian/DM hotbars and soundboard in a live local room; zero console/page errors.
- Not done: no drag-and-drop automated test coverage (native HTML5 DnD is unreliable to simulate in headless Playwright; verified structurally/visually instead). No portable EXE built locally; CI covers that on push.
...
- Native desktop now opens two synced windows using `surface=hotbar` / `surface=table`. The hotbar has action editing and roll controls; the table has dice, console and result reveal. The hotbar never creates a duplicate dice renderer. Both share the persistent session, saved actions and room polling. Credential changes propagate between windows.
- Root cause of lost click-through: the old host explicitly called `setIgnoreMouseEvents(false)` on its whole overlay. The table now starts with native click-through and focus disabled. The hotbar remains interactive. Its host-owned **Interact with table / Lock table** button, local controls checkbox, tray menu and Ctrl+Shift+T toggle native table input. No general remote Electron bridge is exposed; IPC checks sender, main frame, origin and value type.
- Both windows show/hide/reconnect together, remain within the selected monitor, recover together from a renderer crash, and stop old-room polling when leaving. Only the table is audible while visible; hotbar audio is always muted. Resize and drag remain independent.
- Version 0.7.2 updates web and desktop manifests/lockfile. Existing EXEs retain their old window architecture; users need the new portable EXE for the split windows.
- Verification: 32 web unit tests, 8 desktop host/config tests, TypeScript and production build pass. All 10 browser tests pass, including split-window action/roll synchronization and existing throw continuity/styles/reveal coverage. Desktop screenshots inspected. Actual Electron verification against a local test room passes Windows WS_EX_TRANSPARENT flags, focus changes, host-button toggling and shared rolls. Windows portable packaging and actual EXE launch/preload smoke check pass. Local evidence: work/split-browser.log, work/native-result.json, work/native-test.log, work/desktop-package.log, work/portable-smoke.log.
- Main delivery `f4a2408` was pushed. Its first Linux CI run caught a missing JSON import attribute in the browser test version check; the test now reads package.json through node:fs, compatible with both loader modes. Follow-up CI must pass before claiming production delivery. Local runtime required removing inherited ELECTRON_RUN_AS_NODE and making npm available on PATH for packaging; source needs no workaround for those machine settings.
- Keep the unrelated Word lock-file deletion unstaged. Never commit work/, native test profiles, recordings, build outputs or credentials.

# Active work — dice styles and throw continuity (2026-09-13)

- Historical branch checkpoint (superseded by the 2026-09-14 delivery below): `feature/dice-styles-and-throw-continuity`. Checkpoint `62390c7`; merge `677f683` incorporates origin/main `25a0c0b`, including Claude’s RPG HUD and independent button colors. ResizablePanel and RollTaskbar remain removed.
