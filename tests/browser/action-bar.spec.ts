...
test('characters migrate, edit, reorder, transfer and roll across browser and desktop', async ({
  page,
  context,
}) => {
  ...
  await page.evaluate(() =>
    localStorage.setItem(
      'rollparty:presets:v1',
      JSON.stringify({
        version: 1,
        presets: [
          { id: 'one', name: 'Sword', expression: '1d20+5' },
          { id: 'two', name: 'Perception', expression: '1d20+3' },
        ],
      }),
    ),
  );
  ...
  await expect(
    bar.getByRole('button', { name: 'Roll Sword', exact: true }),
  ).toBeVisible();
  ...
  await bar
    ...
    .click();
  await expect(bar.locator('.saved-action').first()).toHaveText(
    'Perception1d20+3',
  );
  await expect(
    overlay.getByRole('button', { name: 'Roll Longsword', exact: true }),
  ).toBeVisible();
  await bar
    .getByRole('button', { name: 'Remove Perception', exact: true })
    .click();
  ...
  await bar
    .getByRole('button', { name: 'Export characters', exact: true })
    .click();
  const download = await downloading;
  const filename = await download.path();
  const before = await page.evaluate(() =>
    localStorage.getItem('rollparty:actions:v1'),
  );
  await bar
    .getByLabel('Import character file', { exact: true })
    .setInputFiles({
      name: 'broken.json',
  ...
  expect(
    await page.evaluate(() => localStorage.getItem('rollparty:actions:v1')),
  ).toBe(before);
  ...
  await expect(
    bar.getByLabel('Character', { exact: true }).locator('option'),
  ).toHaveCount(3);
  await expect(
  ...
  await expect(page.locator('.roll-entry').first()).toContainText(
    'Action tester',
  );
  await page.reload();
  await expect(
  ...
  expect(
    await page.evaluate(
      () =>
        JSON.parse(localStorage.getItem('rollparty:presets:v1')!).presets[0]
          .name,
    ),
  ).toBe('Sword');
  await bar.locator('summary').click();
  await bar
    ...
    .click();
  await bar
    .getByRole('button', { name: 'Keep character', exact: true })
    .click();
  await expect(
    bar.getByLabel('Character', { exact: true }).locator('option'),
  ).toHaveCount(3);
  for (const width of [1440, 390]) {
    ...
    await page.screenshot({
      path: `test-results/action-bar-${width}.png`,
      fullPage: true,
    });
  }
  await overlay
