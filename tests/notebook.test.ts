...
const roll: Roll = {
  ...
  dice: [
    ...
    { sides: 20, value: 14, kept: true },
  ],
  modifier: 2,
  total: 16,
  label: 'Attack',
  created: 1788984000000,
};
test('notebook keeps only the current player and preserves comments through repeated history refreshes', () => {
  const notes = mergeNotes(
    [],
    [roll, { ...roll, id: 'two', playerId: 'other' }],
    'me',
  );
  ...
  assert.deepEqual(
    readNotes(JSON.stringify({ version: 1, entries: notes }), 'me'),
    notes,
  );
  assert.deepEqual(
    readNotes(JSON.stringify({ version: 1, entries: notes }), 'other'),
    [],
  );
...
test('Markdown retains linked damage and critical context', () => {
  const notes = mergeNotes(
    [],
    [
      roll,
  ...
  assert(notesMarkdown(notes).includes('Damage for: Attack (one) - critical'));
});
